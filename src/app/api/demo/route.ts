...
async function clearDemoWorkoutHistory(supabase: SupabaseClient, userId: string): Promise<void> {
  ...
  if (error) {
    throw new Error(error.message || 'Failed to clear existing demo workout history');
  }
}
...
export async function POST() {
  try {
    ...
    await clearDemoWorkoutHistory(supabase, userId);
